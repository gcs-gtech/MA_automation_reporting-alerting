...
def main(client, customer_id,  page_size):
    ...
    query = ('SELECT campaign.id, ad_group_ad.ad.id, ad_group_ad.ad.type, '
    ...
    try:
        ...
        for row in results:
            ...
            d_series = pd.Series([campaign_id, ad.id.value, ad_type_enum.Name(ad.type), policy_summary.policy_topic_entries[0].topic.value, policy_topic_entry_type_enum.Name(policy_summary.policy_topic_entries[0].type)], 
                index=['campaign_id', 'ad_id', 'ad_type', 'policy_topic', 'policy_topic_type'])
            d.append(d_series)
